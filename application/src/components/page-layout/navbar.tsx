...
const NavLinkStyled:
    FunctionComponent<NavLinkProps> = ({to, exact, children}) =>
        <NavLink {...exact} to={to} className={styles.navLink}>{children}</NavLink>

// Procedurally create all navigation bar elements from the routesMap
const routes = Object.values<RouteObject>(routesMap)
.map(({path, visibleName}) => {
        switch(visibleName) {
